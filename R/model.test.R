...
model.test <- function(data, models, pool.variance = NULL, time.split, fixed.optima = FALSE, control.list = list(fnscale = -1), verbose = TRUE) {
    ...
    ## Running the models
    models_out <- lapply(1:n_models, lapply.models, models, model_test_input, time.split, control.list, fixed.optima, verbose)
    ...
    output <- list("aic.models" = cbind(aicc, delta_aicc, weight_aicc), "full.details" = models_out)
